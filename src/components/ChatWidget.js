...
export default function ChatWidget() {
  ...
  return (
    <div className="fixed bottom-4 right-4 md:bottom-8 md:right-8 z-50">
      {isOpen && (
        <div className="bg-white border-2 border-brand-dark rounded-3xl shadow-2xl w-80 sm:w-96 flex flex-col mb-4 overflow-hidden h-[500px] animate-in fade-in slide-in-from-bottom-4">
          <div className="bg-brand-dark text-brand-volt p-4 flex justify-between items-center">
            <h3 className="font-black uppercase tracking-tighter italic text-sm">Locker Room</h3>
            <button onClick={() => setIsOpen(false)} className="text-white hover:text-brand-volt transition-colors">
          ...
          <div className="flex-grow p-4 overflow-y-auto bg-slate-50 space-y-4">
            {messages.map((msg) => {
              ...
              if (msg.message_type === 'system_alert') {
                return (
                  <div key={msg.id} className="flex justify-center my-2"><div className="bg-brand-volt text-brand-dark px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border border-brand-dark/20">📢 {msg.content}</div></div>
                );
              }
              return (
                <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                  <span className="text-[10px] font-black text-gray-400 uppercase mb-1 px-1">{isMe ? 'You' : msg.author_name}</span>
                  <div className={`max-w-[85%] px-4 py-2 rounded-2xl text-sm shadow-sm ${isMe ? 'bg-brand-violet text-white rounded-tr-none' : 'bg-white border border-gray-200 text-brand-dark rounded-tl-none'}`}>
                    {msg.content}
                  </div>
                </div>
              );
          ...
          <form onSubmit={sendMessage} className="p-4 bg-white border-t border-gray-100 flex gap-2">
            ...
            <button type="submit" className="bg-brand-dark text-brand-volt p-2 rounded-full hover:scale-110 transition-transform">
      ...
      {!isOpen && (
        <button onClick={() => setIsOpen(true)} className="bg-brand-dark text-brand-volt h-14 w-14 rounded-full shadow-lg flex items-center justify-center hover:scale-110 transition-all border-2 border-brand-volt/20">
